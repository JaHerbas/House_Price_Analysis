...
Three variables/features were chosen initially as part of our hypothesis, these were: the sector (location based on lat and long), square foot of living (sqft_living) and the bedrooms. The final model however, doesn't support all of these three as the most predictive ones. The bedrooms increase the price of the property up to a certain point when they start decreasing the property. Because I created sectors, the results vary among each so it is hard to come up with a specific number, but by looking at the plots from the notebooks it becomes clear that the trend is to decrease, so we can't consider this one.

The sqft of living is the most predictive variable as the model shows. Sector 1 is a very good predictor as well but the others don't seem to be that great. Bedrooms aren't good, whereas bathrooms are, which is strange. I'm not sure how to explain this. Grade_C is another top predictor but this is expected as it comprises very good grades: 9 to 11 out of a range from 3 to 13, so clearly expensive homes will be within this range.  
...
Other variables were studied and considered as potentially having an impact on the model's prediction capability, but due to time constrain no more work was done on them. Those variable are the Condition and of course the zipcode, which as mentioned at the beginning, it is believed to be the strongest price predictor and could bee interesting to re-run the model using this as a categorical. 

With previous explanation, a list of what I consider the best predictors is below:
* sqft_living
* sqft_living15
* waterfront
* Grade_C
...
With the OLS Regression model results, further EDA and data engineering was done for some of these categories plus others so that the message could be easier transfered to out Stakeholders.
...
* Concentrate on Sectors 1 and 2 for high end strong investment with average home price between 600 to 800,000 Dollars. This gives you the flexibility to buy in the waterfront or inland, with a good living size within the Mid Class average for Seattle (2500 sqft)

* For lower average price homes, go to sector 4. There are still some waterfront homes with lower price than in sector 1 and 2, and for an average of 500,000 Dollars you should get a property within the 2500 sqft
